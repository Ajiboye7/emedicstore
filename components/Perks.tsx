...
const Perks = () => {
  return (
    ...
    <div className="flex flex-col md:flex-row items-start gap-8 mb-8">
      ...
      <div className="w-full md:w-1/3">
        <div className="bg-white shadow-md p-6 rounded-lg">
          <h3 className="text-2xl font-bold mb-4" style={{ color: Perks[0].color }}>{Perks[0].title}</h3>
          <p className="text-bodyText mb-4">{Perks[0].body}</p>
        </div>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {Perks.slice(1).map((perk) => (
        <div key={perk.id} className="bg-white shadow-md p-6 rounded-lg">
          <h3 className="text-2xl font-bold mb-4" style={{ color: perk.color }}>{perk.title}</h3>
          <p className="text-bodyText mb-4">{perk.content}</p>
        </div>
      ))}
    </div>
  </section>
